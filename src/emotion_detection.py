...
class App:
    ...
    def __init__(self, emotion_recognition_model_filepath: str, age_recognition_model_filepath: str) -> None:
        """Initialize the application."""
        self._emotion_recognition_model = self._load_model(emotion_recognition_model_filepath)
        self._age_recognition_model = self._load_model(age_recognition_model_filepath)
        ...
        while True:
            ...
            if not isinstance(faces_detected, tuple):
                for (x, y, w, h) in faces_detected:
                    cv2.rectangle(img, (x, y), (x + w, y + h), (153, 153, 153), thickness=2)
                    ...
                    for i, line in enumerate(text):
                        iy = y0 + i * dy
                        cv2.putText(img, line, (int(x), iy), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 171, 240), 2)
